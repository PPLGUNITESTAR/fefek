...
static int soundbus_device_suspend(struct device *dev, pm_message_t state)
{
	...
	if (dev->driver) {
		struct soundbus_driver * drv = to_soundbus_driver(dev->driver);
		if (drv->suspend) {
			int err = drv->suspend(soundbus_dev, state);
			if (err)
				return err;
		}
	}

	if (soundbus_dev->suspend)
		return soundbus_dev->suspend(soundbus_dev, state);
	return 0;
}
...
static int soundbus_device_resume(struct device *dev)
{
	...
	if (dev->driver && drv->resume) {
		int err = drv->resume(soundbus_dev);
		if (err)
			return err;
	}

	if (soundbus_dev->resume)
		return soundbus_dev->resume(soundbus_dev);
	return 0;
}
#endif /* CONFIG_PM */
